...
int main(int argc, char *argv[]) {
	...
	if(connect(udp_client_socket, (struct sockaddr *) &udp_server_endpoint, udp_server_endpoint_length)==-1){
	...
	// UDP IPv4: "send" para o servidor
	printf("a enviar dados para o servidor... "); 
	fflush(stdout);
	if ((udp_sent_bytes = send(udp_client_socket, &number, sizeof(number), 0)) == -1)
		ERROR(24, "Can't send server");
	printf("ok.  (%d bytes sent)\n", (int)udp_sent_bytes);

	
	char response[MAX_MSG_SIZE];

	// UDP IPv4: "recv" do servidor (bloqueante)
	printf("à espera de dados do servidor... "); fflush(stdout);
	if ((udp_read_bytes = recv(udp_client_socket, response, sizeof(response), 0)) == -1)
		ERROR(25, "Can't recv server");
